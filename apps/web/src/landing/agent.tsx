...
export interface AgentState {
  ...
  trail: { id: number; points: { x: number; y: number }[] };
}
...
function arc(from: { x: number; y: number }, to: { x: number; y: number }, n = 9) {
...
const set = (patch: Partial<AgentState>) => {
  state = { ...state, ...patch };
...
export function take(): Script {
  const me = ++holder;
  const alive = () => {
    if (me !== holder) throw new Cancelled();
  };
  const wait = (ms: number) =>
    new Promise<void>((resolve, reject) =>
      setTimeout(() => (me === holder ? resolve() : reject(new Cancelled())), reducedMotion() ? 0 : ms),
    );
  return {
...
export function Agent() {
  ...
  return (
    <>
      <div key={a.trail.id} className="jev-trail" aria-hidden>
        {a.trail.points.map((p, i) => (
          <i
            // biome-ignore lint/suspicious/noArrayIndexKey: positions along one path
            key={i}
            style={{ left: p.x, top: p.y, animationDelay: `${i * 45}ms` }}
          />
        ))}
      </div>
      {a.ring && (
        <div
          ...
          style={{ left: a.ring.x - 6, top: a.ring.y - 6, width: a.ring.w + 12, height: a.ring.h + 12 }}
          aria-hidden
        />
      )}
      <div
        className={`jev ${a.visible ? "on" : ""} mood-${a.mood}`}
        style={{ transform: `translate(${a.x}px, ${a.y}px)` }}
        aria-hidden
      >
        <span key={a.ripple} className={a.ripple ? "jev-ripple go" : "jev-ripple"} />
        <svg className="jev-arrow" width="30" height="30" viewBox="0 0 26 26">
          <path
          ...
          />
          <circle className="jev-spark" cx="20.5" cy="5.5" r="2.8" fill="#ff8a00" stroke="#fff" strokeWidth="1.2" />
        </svg>
        {a.label && <span className="jev-says">{a.label}</span>}
      </div>
    </>
  );
}
