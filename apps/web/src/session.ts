import { ApiClient, type Me, type TokenStore, type Tokens } from "@jev-browser/protocol";
...
async function postAuth(path: string, body: object): Promise<Response> {
  return fetch(path, {
    method: "POST",
    credentials: "include",
...
export async function restoreSession(): Promise<boolean> {
  ...
  if (!document.cookie.split("; ").includes("jev_signed_in=1")) return false;
...
export async function authConfig(): Promise<{ google: boolean; dev: boolean }> {
  const res = await fetch("/auth/config").catch(() => null);
  if (!res?.ok) return { google: false, dev: false };
  return (await res.json()) as { google: boolean; dev: boolean };
}

export function signInWithGoogle(next = "/dashboard"): void {
...
export async function signInDev(email: string): Promise<void> {
  ...
  if (!res.ok) throw new Error("Development sign-in is not available on this server.");
  const body = (await res.json()) as { access: string };
  await store.set({ access: body.access, refresh: "" });
}
...
export type SessionState =
  | { status: "loading" }
  | { status: "signedOut" }
  | { status: "signedIn"; me: Me; refresh: () => Promise<void> };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (alive) setState({ status: "signedIn", me, refresh: load });
      } catch {
        if (alive) setState({ status: "signedOut" });
      }
    };
    void load();
    listeners.add(load);
